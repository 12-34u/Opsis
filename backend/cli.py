...
from assembler_engine import AssemblerEngine
...
def main():
    ...
    # Assemble
    try:
        engine = AssemblerEngine(isa_data if isa_data else args.isa)
        result = engine.assemble(source, args.origin)
        ...
        if args.format in ('hex', 'all'):
            Path(f"{output_stem}.hex").write_text(engine.get_hex_dump())
            print(f"Wrote {output_stem}.hex")
        
        if args.format in ('lst', 'all'):
            Path(f"{output_stem}.lst").write_text(engine.get_listing())
