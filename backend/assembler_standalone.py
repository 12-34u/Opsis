        ...
        while pos < len(line):
            ...
            if line[pos:pos+2].lower() in ('0b', '0B'):
                ...
                if match:
                    ...
                    pos += len(match.group(0))
                    continue
            # Hex number with h suffix (e.g., 4Ch, 0FFh) - must check before decimal
            if line[pos].isdigit():
                # Try hex with h suffix first (4Ch, 0FFFFh, etc.)
                hex_match = re.match(r'[0-9][0-9A-Fa-f]*[hH]', line[pos:])
                if hex_match:
                    tokens.append(Token(TokenType.IMMEDIATE_VALUE, hex_match.group(0), line_num, pos + 1))
                    pos += len(hex_match.group(0))
                    continue
            if line[pos].isdigit() or (line[pos] == '-' and pos + 1 < len(line) and line[pos+1].isdigit()):
                # Decimal, binary, or octal
                match = re.match(r'-?\d+[bBdDoO]?', line[pos:])
                if match:
                    tokens.append(Token(TokenType.IMMEDIATE_VALUE, match.group(0), line_num, pos + 1))
                    pos += len(match.group(0))
                    continue
